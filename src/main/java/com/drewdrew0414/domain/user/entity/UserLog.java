...
public class UserLog {
    ...
    private UserLog(Long userId, String attemptName, String attemptEmail, LoginStatus status, String clientIp, String userAgent) {
        ...
        // 클라이언트가 자유롭게 늘릴 수 있는 부가 정보 때문에 INSERT가 실패하면
        // 로그인 실패 횟수와 감사 로그까지 함께 롤백됩니다. 컬럼 용량 안에서만 보관합니다.
        // 유니코드 보조 문자 중간을 자르지 않도록 UTF-16 길이가 아닌 코드 포인트로 셉니다.
        this.userAgent = userAgent == null ? null : userAgent.substring(0,
                userAgent.offsetByCodePoints(0, Math.min(255, userAgent.codePointCount(0, userAgent.length()))));
    }
